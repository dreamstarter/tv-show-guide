...
export class ShowValidationError extends Error {
  public override readonly name = 'ShowValidationError';
...
export class ShowOperationError extends Error {
  public override readonly name = 'ShowOperationError';
...
export const VALIDATION_RULES = {
  ...
  NETWORKS: ['ABC', 'NBC', 'CBS', 'FOX'] as const,
...
export const validators = {
   ...
   */
  validateShow(show: unknown): ValidationResult {
    const errors: ShowValidationError[] = [];
    ...
    try {
      ...
      // Validate title
      const titleValidation = this.validateTitle(showObj.t);
      if (!titleValidation.isValid) {
        errors.push(...titleValidation.errors);
      }

      // Validate platform
      const platformValidation = this.validatePlatform(showObj.c);
      if (!platformValidation.isValid) {
        errors.push(...platformValidation.errors);
      }

      // Validate network
      const networkValidation = this.validateNetwork(showObj.net);
      if (!networkValidation.isValid) {
        errors.push(...networkValidation.errors);
      }

      // Validate air day (optional)
      if (showObj.air !== undefined && showObj.air !== null && showObj.air !== '') {
        const airDayValidation = this.validateAirDay(showObj.air);
      ...
      if (showObj.s !== undefined && showObj.s !== null && showObj.s !== '') {
        const seasonValidation = this.validateSeason(showObj.s);
      ...
      if (showObj.eps !== undefined && showObj.eps !== null && showObj.eps !== '') {
        const episodesValidation = this.validateEpisodes(showObj.eps);
      ...
      if (showObj.start) {
        const startDateValidation = this.validateDate(showObj.start, 'start');
      ...
      if (showObj.end) {
        const endDateValidation = this.validateDate(showObj.end, 'end');
      ...
      if (showObj.start && showObj.end) {
        const dateLogicValidation = this.validateDateLogic(showObj.start as string, showObj.end as string);
      ...
      if (showObj.ret !== undefined) {
        const returningValidation = this.validateReturning(showObj.ret);
   ...
   */
  validateTitle(title: unknown): ValidationResult {
    const errors: ShowValidationError[] = [];
   ...
   * Validates platform
   */
  validatePlatform(platform: unknown): ValidationResult {
    const errors: ShowValidationError[] = [];
    ...
    } else if (!VALIDATION_RULES.PLATFORMS.includes(platform as Platform)) {
      errors.push(new ShowValidationError(
   ...
   * Validates network
   */
  validateNetwork(network: unknown): ValidationResult {
    const errors: ShowValidationError[] = [];
    ...
    } else if (!VALIDATION_RULES.NETWORKS.includes(network as Network)) {
      errors.push(new ShowValidationError(
   ...
   * Validates air day
   */
  validateAirDay(airDay: unknown): ValidationResult {
    const errors: ShowValidationError[] = [];

    if (airDay && !VALIDATION_RULES.DAYS.includes(airDay as AirDay)) {
      errors.push(new ShowValidationError(
   ...
   */
  validateSeason(season: unknown): ValidationResult {
    const errors: ShowValidationError[] = [];

    if (season !== null && season !== undefined && season !== '') {
      const seasonNum = typeof season === 'string' ? parseInt(season, 10) : season as number;
   ...
   */
  validateEpisodes(episodes: unknown): ValidationResult {
    const errors: ShowValidationError[] = [];

    if (episodes !== null && episodes !== undefined && episodes !== '') {
      const episodeNum = typeof episodes === 'string' ? parseInt(episodes, 10) : episodes as number;
   ...
   */
  validateDate(date: unknown, fieldName = 'date'): ValidationResult {
    const errors: ShowValidationError[] = [];
   ...
   */
  validateDateLogic(startDate: string, endDate: string): ValidationResult {
    const errors: ShowValidationError[] = [];
   ...
   */
  validateReturning(returning: unknown): ValidationResult {
    const errors: ShowValidationError[] = [];
   ...
   * Validates show ID
   */
  validateShowId(id: unknown): ValidationResult {
    const errors: ShowValidationError[] = [];
    ...
    if (id === undefined || id === null) {
      ...
      const idNum = typeof id === 'string' ? parseInt(id, 10) : id as number;
  ...
  validateImportData(data: unknown): ValidationResult {
    ...
    Object.entries(dataObj).forEach(([id, show]) => {
...
export class ErrorHandler {
   ...
   */
  handleError(error: Error, showToUser = true, context: unknown = null): ErrorLogEntry {
    // Log the error
   ...
   */
  private displayErrorToUser(error: Error, context: unknown = null): void {
   ...
   */
  private showNotification(message: string, type: NotificationType = 'info'): boolean {
...
export const safeOperations = {
  ...
  async safeAsync<T>(
    ...
    try {
      ...
      if (errorHandler) {
        errorHandler.handleError(err, true);
      } else {
   ...
   */
  safeLocalStorage: {
    getItem(key: string, errorHandler: ErrorHandler | null = null): string | null {
      try {
        return localStorage.getItem(key);
      } catch (error) {
        const err = error instanceof Error ? error : new Error(String(error));
        const operationError = new ShowOperationError(
          `Failed to read from localStorage: ${err.message}`,
    ...
    setItem(key: string, value: string, errorHandler: ErrorHandler | null = null): boolean {
      try {
        localStorage.setItem(key, value);
        return true;
      } catch (error) {
        const err = error instanceof Error ? error : new Error(String(error));
        let errorCode: ErrorCode = ERROR_CODES.OPERATION.SAVE_FAILED;
    ...
    removeItem(key: string, errorHandler: ErrorHandler | null = null): boolean {
      try {
        ...
        const err = error instanceof Error ? error : new Error(String(error));
        const operationError = new ShowOperationError(
          `Failed to remove from localStorage: ${err.message}`,
...
declare global {
  interface Window {
    ShowValidation: {
      ...
      safeOperations: typeof safeOperations;
      ShowValidationError: typeof ShowValidationError;
      ShowOperationError: typeof ShowOperationError;
